...
public class SequenceDatabaseWithUtility {
	...
	void addSequence(String[] tokens) { 
		...
		// The reason why we count this, is that HUSRM and US-Rule do not allow items to appear twice 
		// or more in the same sequence. Thus, we have to count the extra utility of items appearing more 
		// than once and subtract this utility from the utility of the sequence and remove these 
		// extra occurrences of items.
		...
		for (String token : tokens) {
			...
			if( token.charAt(0) == 'S') {
				...
				setSumUtil(getSumUtil() + sequence.exactUtility);
			}
